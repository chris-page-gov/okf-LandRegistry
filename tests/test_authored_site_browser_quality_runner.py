...
class RunnerFixture:
    ...
    def _build_candidate(self) -> tuple[str, str]:
        ...
        profile_material = b'{"schema":"fixture"}\n'
        write(profile_root / "fixture.json", profile_material)
        profile_row = {
            "path": "fixture.json",
            "bytes": len(profile_material),
            "sha256": sha256(profile_material),
        }
        aggregate = sha256(
            f"fixture.json\t{len(profile_material)}\t{profile_row['sha256']}\n".encode()
        )
        vendor_lock = {
            "schema": "okf-profile-vendor-lock.v1",
            "profile": "https://example.invalid/profile/bundle-wiki/v1/",
            "release": {
                "repository": "https://example.invalid/explorer",
                "version": "0.6.0",
                "tag": "v0.6.0",
            ...
            "file_count": 1,
            "identity": {"algorithm": "sha256", "sha256": aggregate},
            "files": [profile_row],
        }
        vendor_path = (
            self.candidate
        ...
        write(vendor_path, json_bytes(vendor_lock))
        ...
        with tempfile.TemporaryDirectory(prefix="okf-g6-runner-") as temporary:
            ...
            for limit_name in (
                ...
                with self.subTest(limit_name=limit_name):
                    ...
                    self.assertNotEqual(0, result.returncode)
                    self.assertIn("executable Explorer v0.6.0", result.stderr)
