...
CONTRACT = {
    "lockstep": {
        "controlled_paths": ["scripts/**", "requirements*.txt"],
        "documentation_paths": ["docs/**", "README.md", "CHANGELOG.md"],
...
class DocumentationLockstepTests(unittest.TestCase):
    def test_uncontrolled_change_needs_no_companion_files(self) -> None:
        self.assertEqual(
            ([], [], []), lockstep.lockstep_errors(CONTRACT, {"notes/private.txt"})
        )
    ...
    def test_documentation_without_changelog_fails(self) -> None:
    ...
    def test_complete_lockstep_passes(self) -> None:
        errors, _, documentation = lockstep.lockstep_errors(
            CONTRACT, {"scripts/build.py", "README.md", "CHANGELOG.md"}
        )
        self.assertEqual([], errors)
        self.assertEqual(["CHANGELOG.md", "README.md"], documentation)

    def test_dependency_change_has_no_actor_exemption(self) -> None:
        errors, controlled, _ = lockstep.lockstep_errors(
            CONTRACT, {"requirements-lock.txt"}
        )
        self.assertEqual(["requirements-lock.txt"], controlled)
        self.assertEqual(2, len(errors))
