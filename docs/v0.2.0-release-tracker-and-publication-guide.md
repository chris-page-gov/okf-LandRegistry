...
- Status: **candidate built; release approval and publication not complete**
...
The v0.2.0 software and public bundle have been built. That does not mean the
...
| Candidate commit containing the governed bundle | `9955823144e05cad295dc51ac0558d2f9174a464` |
| Bundle release root | `e07fafe25bbd816f790f6a604aa7e99ab529dda51ec7d106e94d2e57485a217d` |
...
2. obtain independent G5 acceptance for the exact v0.2.0 suite and bundle;
3. create new exact-candidate G1–G8 receipts;
4. obtain the project owner's G9 approval for those exact bytes;
...
| Deterministic v0.2.0 bundle | Complete for the candidate | `bundle/CHECKSUMS.sha256`; root `e07fafe…` |
| Two clean, byte-identical builds | Complete for the candidate | Local candidate validation; must be cited in the new G7 receipt |
| Unit/integration suite | Complete for the candidate | 129 tests passed; must be cited in new receipts |
| Pinned Explorer compatibility | Complete for the candidate | Receipts under `validation/candidate-v0.2.0/` |
| Change-impact classification | Complete | `validation/candidate-v0.2.0/change-impact.json` selects G1–G9 and Stage 1 review |
| Renewed Stage 1 review | **Blocked / not recorded** | Final profile and architecture changes need a post-remediation review attestation |
| Independent v0.2.0 G5 acceptance | **Blocked / not performed** | Calibration passed, but independent hard-failure and held-out review is absent |
| Exact v0.2.0 G1–G8 receipts | **Not assembled** | Checked-in canonical receipts still bind v0.1.0 |
| Independent release recommendation | **Not performed for v0.2.0** | The checked-in recommendation binds v0.1.0 |
| G9 owner approval | **Blocked / not given** | Must follow G1–G8 and name the exact v0.2.0 root, claims, risks and URL |
...
The short form `e07…` is convenient in prose, but approvals and receipts must
use the complete 64-character value.

### Gate

A gate is a release question that must receive a documented pass. For example,
G4 asks whether the data and OKF structures are valid; G5 asks whether the
retrieval behaviour and safety expectations were independently evaluated.
...
The profile text permits building the v0.2.0 candidate, and its JSON and YAML
forms validate and hash correctly. However, the final remediation changed
material parts of the contract after the earlier warm-up work, including:
...
The change-impact result therefore says `stage1_review_required: true`. No
separate post-remediation review attestation currently closes that flag.
Writing “approved” in an authored report is not the same as recording who
reviewed the final contract, what they reviewed, and their decision.
...
- If the reviewer requires a material profile change, rebuild the candidate.
  The `e07…` root and all downstream exact-candidate receipts may then become
  obsolete.
...
7. produce the new exact-candidate G1 evidence only after any required changes
...
## Block 2 — independent G5 acceptance

### Meaning

G5 asks whether users can retrieve the intended authoritative records while
avoiding dangerous near misses. It covers more than whether search returns
something.
...
However, its report correctly says `hard_failures_evaluated: false`. The
calibration checks the retrieval implementation against authored
expectations. It cannot independently verify that those expectations,
near-miss rules and caveats are themselves correct.

The real Explorer receipts prove that the exact consumer can load, search and
restore state. They are compatibility evidence, not an independent judgement
of answer quality or safety.

The checked-in [`evaluation/acceptance-review.json`](../evaluation/acceptance-review.json)
binds the v0.1.0 root `a3c15f0e…`, not the v0.2.0 root `e07fafe…`. Reusing it
would defeat the exact-digest evidence rule.
...
- Passing calibration does not close G5.
...
- If G5 exposes a search or content defect, fix the governed input or
  generator and rebuild. Do not weaken the expected result after seeing the
  failure without independent justification.
- A changed bundle requires a new root and invalidates any acceptance review
  already written for `e07…`.
...
5. create a new v0.2.0 acceptance-review document binding the complete
   `e07…` root and exact question-suite SHA-256;
...
## Required exact-candidate G1–G8 evidence
...
new set of G1–G8 receipts.

The change-impact report lists every gate as `not_run`. That does not mean the
129 tests failed or were not executed. The classifier is a planning tool: by
design, it can select work but cannot claim a pass, reuse evidence or approve
a release.

Before G9, evidence producers must create v0.2.0 artifacts for:

- G1: final profile validation and renewed review;
- G2: frozen snapshot integrity and terminal-outcome reconciliation;
- G3: rights, privacy, prohibited-content and safety review;
- G4: OKF, data, JSON-LD, identity, checksum and pinned-consumer integrity;
- G5: the independent acceptance described above;
- G6: authored-site and Explorer journeys, accessibility, security and
  performance;
- G7: two clean byte-identical builds and reconciled change impact; and
- G8: archive, manifest, dependency/workflow provenance and release metadata.

Each receipt must name the exact candidate commit and `e07…` release root.
Old receipts remain immutable in the v0.1.0 tag and must not be relabelled.

## Block 3 — G9 owner approval

### Meaning

G9 is the project owner's explicit decision to publish one exact candidate. It
...
- the candidate commit and G1–G8 receipt hashes;
...
No owner record approves root `e07…` after reviewing new v0.2.0 G1–G8
evidence, because that evidence cannot be assembled until Stage 1 and G5 are
closed.
...
general instruction to build or prepare a release is not an exact-digest G9
decision.
...
G9 makes responsibility visible and prevents a deployment token or green CI
run from becoming accidental authorisation.

### Implications

- Do not set `OKF_RELEASE_ROOT_SHA256` before G9.
- Approval applies only to the complete `e07…` value. Any governed-byte change
  reopens G9.
...
After reviewing the complete G1–G8 evidence and independent release
recommendation, the project owner must record:
...
```mermaid
flowchart TD
    A["Frozen v0.2.0 candidate<br/>root e07…"] --> B["Renew Stage 1 review"]
    B --> C{"Material change required?"}
    C -- "Yes" --> D["Change governed input<br/>rebuild and obtain a new root"]
    D --> B
    C -- "No" --> E["Produce exact G1–G4 and G6–G8 evidence"]
    E --> F["Independent G5 review and formal evaluation"]
    F --> G{"All G1–G8 pass?"}
    G -- "No" --> H["Stop; record failure<br/>do not publish"]
    G -- "Yes" --> I["Independent release recommendation"]
    I --> J["Owner G9 exact-digest decision"]
    J --> K{"Approved?"}
    K -- "No" --> H
...
documentation-only tracking commit may exist after candidate commit
`9955823…`, but governed candidate paths must not drift.
...
git diff --exit-code 9955823144e05cad295dc51ac0558d2f9174a464 \
...
bytes. The calibration command still does not close G5.
...
Use the v0.2.0 journey manifest:
...
tree `fc11e903…`, a clean consumer source state, the `e07…` bundle identity,
requests, console output, restored state and terminal outcome.
...
Follow Block 2. A formal G5 result must be produced with
`--acceptance-review`. A calibration-only report cannot be cited as a G5 pass.

If G5 fails, report the failure immediately. Do not silently rewrite the
question suite, discard held-out cases or begin a release rebuild outside the
declared change-impact scope.

### Step 4 — create the deterministic archive and supply-chain evidence

After the candidate and G1–G7 evidence are stable:
...
.venv/bin/python scripts/create_release_metadata.py \
  --candidate-commit-sha 9955823144e05cad295dc51ac0558d2f9174a464 \
...
### Step 5 — obtain the independent release recommendation and G9 decision

The independent release reviewer reads the complete G1–G8 evidence and records
a recommendation with limitations. The project owner then makes the G9
decision described in Block 3.
...
Only an authorised repository owner should do this, and only after the G9
record is committed and independently checked:

```bash
gh variable set OKF_RELEASE_ROOT_SHA256 \
  --body e07fafe25bbd816f790f6a604aa7e99ab529dda51ec7d106e94d2e57485a217d \
...
```bash
OKF_RELEASE_ROOT_SHA256=e07fafe25bbd816f790f6a604aa7e99ab529dda51ec7d106e94d2e57485a217d \
...
- unchanged `e07…` bundle bytes;
- new v0.2.0 G1–G8 receipts;
...
- the full root in deployed `CHECKSUMS.sha256` equals `e07…`;
...
6. if governed bytes change, create a new root, repeat affected gates, obtain
   a new G9 decision and publish a new RC.
...
- [`release-assurance.md`](release-assurance.md) — normative G0–G9 contract.
