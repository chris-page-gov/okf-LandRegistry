...
| Candidate commit containing the governed bundle | `8a60539d4fce183aec2d0e7265573027657159ab` |
| Bundle release root | `0fdab21a1f48b66168be7a03dc673305207b41ac526fe98d10a0594a64920b6f` |
...
evaluator now reproduces the pinned consumer's ordering. Because its own hash
is recorded in the build receipt, the correction produced current root
`0fdab21a…`. Both exact browser suites were rerun and now agree on consumer
tree `91bc8aca…`. A narrow independent addendum is still required; authored
root-cause analysis cannot convert Claude's explicit failures into passes.
...
| Deterministic v0.2.0 bundle | Complete for the candidate | `bundle/CHECKSUMS.sha256`; root `0fdab21a…` |
...
| Unit/integration suite | Complete for the candidate | 136 tests passed; must be cited in new receipts |
| Pinned Explorer search compatibility | Complete for the candidate | 26/26 journeys passed in `explorer-search-runtime-collation-fixed-0fdab21a.json` |
| Pinned Explorer product compatibility | Complete for the candidate | 6/6 journeys passed in `explorer-product-runtime-collation-fixed-0fdab21a.json` |
...
| Change-impact classification | Complete, manual review required | `change-impact-collation-fix-8a60539.json` selects affected work and deliberately prevents new evidence from approving itself |
| Renewed Stage 1 review | **Blocked / addendum required** | Claude rejected `50506bff…` only on the runtime-tree mismatch; it must independently verify the correction and bind a decision to `0fdab21a…` |
| Independent v0.2.0 Land Registry G5 acceptance | **Blocked / addendum required** | The corrected exact-runtime evidence passes locally, but Claude's submitted G5 decision is still explicitly `fail` for another root |
...
The short form `0fdab21a…` is convenient in prose, but approvals and receipts must
...
the bundle root. The change-impact result consequently still says
`stage1_review_required: true`. Claude's second `fail` remains valuable
historical evidence but cannot close that flag for root `0fdab21a…`. Writing
“root cause fixed” in an authored report is not the same as the reviewer
verifying the correction and issuing a digest-bound decision.
...
- If the reviewer requires a material profile change, rebuild the candidate.
  The `0fdab21a…` root and all downstream exact-candidate receipts may then become
  obsolete.
...
product journeys also pass. Both receipts bind tree `91bc8aca…`, and the
corrected formal evaluator recomputes the same tree locally. This is strong
executable evidence, but it remains evidence for Claude's addendum rather than
a self-authored G5 decision.

The checked-in [`evaluation/acceptance-review.json`](../evaluation/acceptance-review.json)
binds v0.1.0. Claude's first failed review binds `e07…`, and its second failed
review binds `50506bff…`. Neither is a pass for `0fdab21a…`.
...
5. create a new v0.2.0 acceptance-review document binding the complete
   `0fdab21a…` root and exact question-suite SHA-256;
...
.venv/bin/python scripts/evaluate.py \
  ...
  --runtime-receipt \
    validation/candidate-v0.2.0/explorer-search-runtime-collation-fixed-0fdab21a.json \
...
136 tests failed or were not executed. The classifier is a planning tool: by
...
No owner record approves root `0fdab21a…` after reviewing new v0.2.0 Land Registry
...
- Approval applies only to the complete `0fdab21a…` value. Any governed-byte change
...
```mermaid
flowchart TD
    A["Frozen v0.2.0 candidate<br/>root 0fdab21a…"] --> B["Renew Stage 1 review"]
...
`8a60539…`, but governed candidate paths must not drift.
...
git diff --exit-code 8a60539d4fce183aec2d0e7265573027657159ab \
...
downstream example of `0fdab21a…`.
...
tree `fc11e903…`, a clean consumer source state, release root `0fdab21a…`,
consumer tree `91bc8aca…`, requests, console output, restored state and
terminal outcome.
...
For the current candidate, use the bounded packet under
[`review/claude-addendum-v0.2.0-0fdab21a/`](../review/claude-addendum-v0.2.0-0fdab21a/).
It asks the same Claude reviewer to reproduce the collation root cause,
confirm the correction boundary, bind complete Stage 1 and G5 decisions to
`0fdab21a…`, and execute the formal evaluator. It explicitly prevents an
authored root-cause note or a copied status field from becoming approval.
...
.venv/bin/python scripts/create_release_metadata.py \
  --candidate-commit-sha 8a60539d4fce183aec2d0e7265573027657159ab \
...
gh variable set OKF_RELEASE_ROOT_SHA256 \
  --body 0fdab21a1f48b66168be7a03dc673305207b41ac526fe98d10a0594a64920b6f \
...
```bash
OKF_RELEASE_ROOT_SHA256=0fdab21a1f48b66168be7a03dc673305207b41ac526fe98d10a0594a64920b6f \
...
- unchanged `0fdab21a…` bundle bytes;
...
- the full root in deployed `CHECKSUMS.sha256` equals `0fdab21a…`;
