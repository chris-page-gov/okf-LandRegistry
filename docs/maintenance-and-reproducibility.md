...
Explorer v0.6.2 consumer lock. The registry profile source remains frozen at
Explorer v0.6.1. The build never fetches a schema or context during the
...
### Routine repository update path

Do not turn every merge into a bundle release. A documentation, navigation or
minor repository correction can use a bounded routine path when the impact
report proves that no causal build input or generated path changed, predicts no
generated output, selects no Stage 1 review and contains no unknown path. The
maintainer must also confirm that the change does not materially alter scope,
authority, rights, public fields, architecture, release claims or approval
state. A filename extension alone is not proof of low impact.

For an eligible routine update:

1. retain the previously approved bundle root unchanged;
2. run the focused tests selected by the report, such as documentation links or
   traceability;
3. do not rebuild `bundle/`, assemble G1–G9 evidence, deploy an RC or request
   owner approval solely because the repository commit changed; and
4. describe the merge as later repository maintenance, not as a changed or
   reapproved v0.3.0 candidate.

This is the Land Registry application of Explorer's reviewed
`documentation-only` rebind and general impact-planning pattern: unchanged
roots are reused and only the affected downstream checks run. If maintainers
later propose the changed repository bytes as a release package, classify that
publication as a separate operation. Existing exact-digest approval never
transfers to the new commit.

The classifier's `release_gates` and `release_approval_required` fields describe
that hypothetical candidate or publication path. They remain `not_run` and may
include G9 for a repository document because the classifier cannot know the
maintainer's later publication intent. They do not force an ordinary
repository-maintenance merge to become a release. The CI routine lane is
narrower: every matched stage must be `documentation`, and every causal,
generated, Stage 1 and unknown-impact condition above must be false. Release
runbooks and other overlapping controlled stages therefore fail closed to the
full lane.

### Full post-release maintenance path

A release-assurance script, workflow or test can require the full local suite
without changing the released candidate. For that bounded case, CI must prove
all of the following before it treats the work as post-release maintenance:

- the impact report requires neither Stage 1 nor manual review;
- no matched stage reports a causal build input;
- no generated path is present in the authored diff; and
- no commit after the evidence anchor changes `bundle/`, `validation/`, the
  source and domain-profile packs, governance,
  personas, evaluation fixtures, contracts, schemas and build-environment
  locks, and those paths remain byte-identical to the final v0.3.0 evidence
  commit.

The full build, deterministic tests, calibration and committed-bundle checks
still run. The G1–G9 closure is then checked against exact evidence commit
`1d708e39f2cde19610d43c5a7f5e36e4a2f947bc`, which must remain the target of
tag `v0.3.0` and an ancestor of current `HEAD`:

```console
.venv/bin/python -B scripts/check_release_evidence.py \
  --repository-root <detached-worktree-at-1d708e39> \
  --manifest validation/candidate-v0.3.0/final-g9/release-evidence.json \
  --evidence-commit-sha 1d708e39f2cde19610d43c5a7f5e36e4a2f947bc
```

CI creates and removes that detached worktree itself. This is necessary because
G8 binds assurance inputs such as the workflow: validating historical G8 from
the later maintenance worktree would correctly report different bytes.

This validates historical evidence; it does not make the maintenance commit
part of the candidate or extend owner approval to repository tooling. If any
protected candidate or evidence path differs, CI uses the candidate-affecting
path and continues to require evidence terminating at current `HEAD`.

The routine path is ineligible when a causal input, generated byte, semantic
decision or controlled meaning changes. In that case follow the candidate
path, rebuild the affected closure and produce fresh evidence. See [Semantic
...
fresh source relationship through the exact Explorer v0.6.2 reader-retention
