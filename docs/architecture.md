...
The v0.3.0 candidate admits only Explorer v0.6.0 and the exact 16-file Bundle
Wiki semantic profile locked in
`contracts/okf-explorer.consumer-lock.json`. The shared semantic assertion
...
The current build receipt records 22,226 direct triples, 22,226 reified
assertions and 22,226 Explorer runtime rows across 13 governed predicates.
The large-corpus relationship runtime covers 6,733 routes using 89 compressed
relationship chunks and a 256-bucket SHA-256 route locator. These are measured
candidate properties, not fixed architectural constants or release evidence.
...
The build still validates every route rather than relying on that heuristic.
It also models Explorer's independent full default-plane loading path. Both
paths are checked against the exact pinned row, chunk, compressed-byte,
decoded-byte and retained-text ceilings, and their measured maxima are written
to `bundle/data/semantic/validation.json`.
...
decisions. CPSV-AP 3.2.0 resources, including its official SHACL shapes, are
vendored and digest-bound. The official shapes have not been run, so the local
bounded checks do not establish CPSV-AP SHACL conformance. Semantic inference
has also not been run; the runtime serves asserted graph material only.
...
this candidate, 42 safe literal-or-final-`/**` patterns expand to exactly 62
...
reproducibility checks. The full candidate-control expansion is 149 files.
This distinction keeps exact build causality separate from exact release
assurance without weakening either.

The graph is not permitted to weaken its own boundary. `scripts/change_impact.py`
contains an independently executable bootstrap for the exact 42 input patterns
...
Explorer `v0.6.0` at the recorded executable commit and exact 16-file semantic
profile lock is admitted. Admitting any other consumer version or profile
identity requires rerunning the positive, degraded and malformed fixtures and
the complete candidate journeys with that exact executable.
