...
- `rights-review.json` records which metadata operations are permitted,
  conditional or prohibited for each source layer; and
...
They are machine-readable inputs, not validation receipts. Their v0.1.0
`released-poc` and policy-approval states authorise only the control design and
release intent. They do not prove that generated bytes passed a gate; only the
exact-digest receipts and G9 release record can do that.
