...
const EXPLORER_V060_RICH_RELATIONSHIP_LIMITS = Object.freeze({
...
const HELP = `Usage:
  node scripts/run_authored_site_browser_quality.mjs \\
    ...
    --explorer-checkout <absolute clean Explorer v0.6.0 checkout> \\
...
// Keep this predicate in exact behavioural parity with Explorer v0.6.0's
...
function exactRichRelationshipLimits(lock) {
  ...
  for (const name of RICH_RELATIONSHIP_LIMIT_NAMES) {
    ...
    invariant(
      limits[name] === EXPLORER_V060_RICH_RELATIONSHIP_LIMITS[name],
      `consumer lock limit ${name} differs from the executable Explorer v0.6.0 contract`
    );
  }
  invariant(
...
async function verifySemanticProfile(repositoryRoot, consumerProfile, consumer) {
  ...
  invariant(vendor.release?.version === consumer.version, 'local semantic profile version differs from consumer lock');
  invariant(vendor.release?.tag === consumer.release_tag, 'local semantic profile tag differs from consumer lock');
  invariant(vendor.release?.commit === consumer.commit_sha, 'local semantic profile commit differs from consumer lock');
...
async function verifyToolchain(explorerRoot) {
  ...
  invariant(appPackage.name === '@okf/explorer' && appPackage.version === '0.6.0', 'Explorer application package identity is not @okf/explorer 0.6.0');
...
async function performPreflight(options) {
  ...
  invariant(consumer.version === '0.6.0' && consumer.release_tag === 'v0.6.0', 'Explorer consumer lock must pin v0.6.0 exactly');
  invariant(COMMIT_PATTERN.test(consumer.commit_sha), 'Explorer consumer commit is invalid');
  invariant(lock.compatibility_window?.policy === 'exact-version-only', 'Explorer compatibility window is not exact-version-only');
  invariant(lock.compatibility_window?.minimum_version === '0.6.0' && lock.compatibility_window?.maximum_version === '0.6.0', 'Explorer compatibility window differs from v0.6.0');
  ...
  const [contractSources, explorerBuild, semanticProfile, toolchain] = await Promise.all([
    ...
    verifySemanticProfile(repositoryRoot, jsonObject(consumer.semantic_profile, 'consumer semantic profile lock'), consumer),
...
function projectRichRelationshipRow(value, plane, label) {
  ...
  invariant(
    Array.isArray(row.evidence) &&
      ...
      row.evidence.length <=
        EXPLORER_V060_RICH_RELATIONSHIP_LIMITS.maximum_rich_relationship_evidence_items,
    ...
    invariant(
      Array.isArray(row.supporting_assertions) &&
        ...
        row.supporting_assertions.length <=
          EXPLORER_V060_RICH_RELATIONSHIP_LIMITS.maximum_rich_relationship_supporting_assertions,
  ...
  if (assertionStatus === 'model-derived') {
    ...
    projected.review_status = requiredRichText(
      row.review_status,
      `${label} review status`
    );
  }
...
function claimBoundary() {
  return {
    ...
    separately_required_receipts: [
      'pinned Explorer v0.6.0 product journeys',
      'pinned Explorer v0.6.0 search-calibration journeys',
...
async function fullRun(options, preflight) {
  ...
  return {
    ...
    limitations: [
      ...
      'Explorer v0.6.0 product and search-calibration receipts remain separately required and are not executed or inferred here.'
    ],
    terminal: {
